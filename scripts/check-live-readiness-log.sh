...
expected_gate_allowed="${VISUAL_HOMING_EXPECTED_LIVE_OUTPUT_GATE_ALLOWED:-0}"
expected_gate_blocked="${VISUAL_HOMING_EXPECTED_LIVE_OUTPUT_GATE_BLOCKED:-150}"
expected_endpoint_stop="${VISUAL_HOMING_EXPECTED_LIVE_ROUTE_ENDPOINT_STOP:-any}"
expected_stop_reason="${VISUAL_HOMING_EXPECTED_LIVE_ROUTE_STOP_REASON:-any}"
...
require_positive_uint() {
    ...
    if [[ ! "${value}" =~ ^[0-9]+$ || "${value}" == "0" ]]; then
        echo "readiness_log_check path=${log_path} passed=false field=${name} expected=positive_uint actual=${value:-missing}" >&2
        return 1
    fi
}
...
check_log() {
    ...
    if [[ "${expected_frames}" == "auto" ]]; then
        ...
        if [[ "${captured_frames}" =~ ^[0-9]+$ && "${requested_frames}" =~ ^[0-9]+$ && "${captured_frames}" -gt "${requested_frames}" ]]; then
            echo "readiness_log_check path=${log_path} passed=false field=frames expected=captured_lte_requested actual=${frames_value}" >&2
            failed=1
        fi
    else
    ...
    if [[ "${expected_stop_reason}" != "any" ]]; then
        require_field "${log_path}" "${compact_line}" "stop_reason" "${expected_stop_reason}" || failed=1
    fi

    if [[ "${expected_gate_reasons}" != "any" ]]; then
        local resolved_gate_reasons="${expected_gate_reasons}"
        if [[ "${expected_gate_reasons}" == *":auto" ]]; then
            resolved_gate_reasons="${expected_gate_reasons%:auto}:${captured_frames}"
        fi
