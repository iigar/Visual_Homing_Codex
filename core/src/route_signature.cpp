...
void write_gray8_pgm(const std::filesystem::path& path, const RouteSignatureEntry& entry, std::uint16_t scale) {
    ...
    if (scale == 0) {
        throw std::runtime_error("Route keyframe scale must be positive");
    }
    ...
    const auto output_width = static_cast<std::size_t>(entry.width) * static_cast<std::size_t>(scale);
    const auto output_height = static_cast<std::size_t>(entry.height) * static_cast<std::size_t>(scale);
    ...
    for (std::size_t y = 0; y < entry.height; ++y) {
        row.clear();
        const auto source_row = y * static_cast<std::size_t>(entry.width);
        for (std::size_t x = 0; x < entry.width; ++x) {
            const auto value = entry.payload[source_row + x];
            for (std::uint16_t repeat_x = 0; repeat_x < scale; ++repeat_x) {
                row.push_back(value);
            }
        }
        for (std::uint16_t repeat_y = 0; repeat_y < scale; ++repeat_y) {
...
std::uint64_t write_route_signature_keyframes(
    ...
    for (const auto& selection : selections) {
        ...
        write_gray8_pgm(output_dir / (std::string(selection.name) + ".pgm"), route.entries[index], scale);
        ++written;
    }
    return written;
}
...
std::uint64_t export_route_signature_keyframes_file(
    ...
    const std::filesystem::path& output_dir,
    std::uint16_t scale) {
    return write_route_signature_keyframes(read_route_signature_file(route_path), output_dir, scale);
}
