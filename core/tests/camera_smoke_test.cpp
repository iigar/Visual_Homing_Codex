...
int main() {
    ...
    bool rejected_match_endpoint_dwell = false;
    try {
        vh::LiveRouteMatchingConfig invalid;
        invalid.route_path = match_route_path;
        invalid.endpoint_dwell_ms = -1.0;
        std::ostringstream ignored;
        (void)vh::match_live_camera_route(invalid, ignored);
    } catch (const std::invalid_argument&) {
    ...
    try {
        vh::LiveRouteMatchingConfig invalid;
        invalid.route_path = match_route_path;
        invalid.endpoint_allow_ambiguous_hold = true;
        std::ostringstream ignored;
        (void)vh::match_live_camera_route(invalid, ignored);
    } catch (const std::invalid_argument&) {
        rejected_match_ambiguous_hold_without_confirmation = true;
    }
    assert(rejected_match_ambiguous_hold_without_confirmation);

    bool rejected_match_ambiguous_hold_dwell = false;
    try {
        vh::LiveRouteMatchingConfig invalid;
        invalid.route_path = match_route_path;
        invalid.endpoint_require_unambiguous_match = true;
        invalid.endpoint_allow_ambiguous_hold = true;
        invalid.endpoint_ambiguous_hold_dwell_ms = -1.0;
        std::ostringstream ignored;
        (void)vh::match_live_camera_route(invalid, ignored);
    } catch (const std::invalid_argument&) {
        rejected_match_ambiguous_hold_dwell = true;
    }
    assert(rejected_match_ambiguous_hold_dwell);

    bool rejected_match_endpoint_stop_frame_dir = false;
    try {
        vh::LiveRouteMatchingConfig invalid;
        invalid.route_path = match_route_path;
        invalid.export_endpoint_stop_frame = true;
        std::ostringstream ignored;
        (void)vh::match_live_camera_route(invalid, ignored);
    } catch (const std::invalid_argument&) {
        rejected_match_endpoint_stop_frame_dir = true;
    }
    assert(rejected_match_endpoint_stop_frame_dir);
