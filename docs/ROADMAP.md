...
- Status: Phase 1 compile-time boundary split, Phase 2 writer-interface scaffolding plus the first concrete serial writer library boundary, Phase 3 runtime/operator/max-limit scaffolding, Phase 4 safety-gate availability wiring, Phase 5 audit-contract extension, and the Phase 6 fail-closed Pi wrapper are implemented. Default builds keep live output disabled, `VISUAL_HOMING_ENABLE_LIVE_MAVLINK_OUTPUT=ON` fails unless paired with `VISUAL_HOMING_ENABLE_BENCH_PROPS_OFF_LIVE_OUTPUT=ON`, and the paired bench-scope build still keeps `VISUAL_HOMING_LIVE_MAVLINK_OUTPUT_AVAILABLE=0`. `LiveMavlinkBridge` now has an injectable `LiveMavlinkCommandWriter` interface tested with fake writers for stopped-send rejection, deterministic double start/stop behavior, and no send when the safety gate blocks. `LiveMavlinkSerialCommandWriter` now encodes MAVLink2 body-frame yaw-rate-only commands through an injectable byte transport, zero-fills but masks velocity fields, and rejects invalid/non-finite/nonzero-velocity/out-of-bound commands before write, but it is not attached to runtime sessions yet. `LiveMavlinkOutputSession` enforces max command count and max duration before writer send, and the Pi wrapper can pass explicit runtime enable plus exact props-off confirmation into the live-route audit path. `LiveMavlinkOutputSafetyGate` blocks `live_output_unavailable` before writer send when runtime-controlled live output is requested while the build remains unavailable. Session audit command records include explicit decision, telemetry, and route-match context, and the audit checker can distinguish expected allowed writer decisions from blocked dry-run decisions. `scripts/run-live-output-bench-props-off-pi.sh` provides the dedicated bench props-off command and currently expects `allowed=0 blocked=150 reason=live_output_unavailable`. Commits `d355bf1` and `6fc9cd2` have accepted `jtzero` fail-closed evidence; the `6fc9cd2` post-writer-library run passed Pi CTest 23/23, captured 150/150 valid runtime route matches, passed directional and endpoint progress, passed telemetry health and dry-run command quality, and session audit reported `allowed=0 blocked=150 reason=live_output_unavailable`. Live MAVLink output remains blocked.
