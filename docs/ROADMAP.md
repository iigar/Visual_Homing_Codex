...
- 2026-07-18 update: the short outdoor route, forward/reverse matching, bounded provider-send evidence, read-only FC baseline, straight-route confirmation, and exact-4.3.6 library-only route-local ODOMETRY encoder now exist. The current next implementation stage is a separate route-local estimator with start-relative vertical state, explicit forward/reverse yaw semantics, reset counter behavior, bounded update-rate/freshness/health gates, and deterministic tests. After that, validate the estimator and exact ODOMETRY contract in SITL without a real FC connection. Do not attach the encoder to writer/session/UART or repeat provider-send until SITL/FC origin/mode acceptance and safety gates are separately reviewed.
