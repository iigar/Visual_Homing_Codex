...
- Status: complete. The external `Visual_Homing_System_Claude-conflict_250226_1330` snapshot was rechecked on `2026-07-18` and is byte-for-byte identical to the tracked `reference/` tree (`242` files, `0` differences). A separate local `_gitnexus_indexed_copy` now provides a clean Git/GitNexus analysis surface without changing the baseline or active core. Future reuse must follow the selective `reuse / rewrite / reject` audit in `docs/REFERENCE_REPOSITORY_AUDIT_UA.md`; no wholesale merge is planned.
...
- Status: started. The trigger evidence is the `2026-06-14` `jtzero` setup attempt where `ExternalNav` was configured but no provider existed, so armed `Guided` still failed with `requires position`. The first implementation step adds a dry-run-only `ExternalNavEstimate` model and route-progress estimator that produces loggable local pose estimates from valid route progress, configured nominal route length, fresh altitude/yaw telemetry, and explicit scale/quality gates. Live route matching can now opt into per-frame `external_nav_estimate` logs through `VISUAL_HOMING_EXTERNAL_NAV_ESTIMATES=1`, with summary counts for FC-ready estimates and invalid reasons. A `2026-06-15` Pi run showed the no-GPS/no-provider FC streaming heartbeat and attitude but no `GLOBAL_POSITION_INT`; the external-nav dry-run match path therefore accepts heartbeat+attitude warmup while still marking estimates invalid unless a relative-altitude source is actually seen. The telemetry inspector now also recognizes MAVLink `ALTITUDE` relative-height payloads. A bench-only `VISUAL_HOMING_EXTERNAL_NAV_BENCH_ALTITUDE_M` fallback can compute diagnostic scale when the FC-reported altitude is zero or negative, but those estimates remain non-FC-ready. With that reference altitude configured, per-frame logs also include camera-only `visual_scale_*` diagnostics from scaled-keyframe comparison; these are for stability analysis only. Two accepted `jtzero` scale-evidence runs used a 150-entry `64x48` route, nominal route length `1.0 m`, bench diagnostic altitude `0.5 m`, 150/150 valid live matches, healthy read-only telemetry with zero dropped bytes, and 150/150 valid visual-scale diagnostics; the first ratio distribution was `0.9..1.15` with most frames at `1.0`, and the repeat run was `0.85..1.15` with 135/150 frames in `0.95..1.05`. Live-route summaries now report aggregate visual-scale validity, ratio min/avg/max, confidence min/avg, a session-level all-valid external-nav readiness gate, a separate log-only external-nav quality gate, altitude-source stability counts/min/avg/max, final telemetry mode/armed state, and an explicit altitude blocker such as `relative_altitude_non_positive`. The quality gate requires visual-scale diagnostics only when the positive bench diagnostic altitude is configured, and summaries report `visual_scale_required` to make that boundary explicit. It can also require an operator-configured expected relative-altitude window to reject physically implausible barometer-origin runs before any external-nav writer exists. A `2026-06-16` Matek H743 Slim V3/SERIAL4 check restored heartbeat+attitude on the Pi UART but still showed no altitude-capable messages, so telemetry inspection now prints a full MAVLink `message_id_counts` histogram to identify the exact stream before changing more SR4 parameters. The next explicit evidence step is a short `5-20 m` outdoor field dry-run: record a fresh route with `scripts/run-field-route-record-pi.sh`, accept it only after route-quality checks pass, then run `scripts/run-external-nav-dry-run-pi.sh` against that route and require current readiness JSON evidence. This remains log-only evidence and does not write `VISION_POSITION_ESTIMATE`, `ODOMETRY`, or any other MAVLink external-nav message.
- 2026-07-18 pre-SITL update: the short outdoor route, forward/reverse matching, bounded provider-send evidence, read-only FC baseline, straight-route confirmation, exact-4.3.6 library-only route-local ODOMETRY encoder, and separate route-local estimator existed. The estimator had explicit route-start vertical state, forward yaw and opt-in reverse-yaw semantics, reset-counter behavior, direction-change reset, bounded update-rate/freshness/progress/health gates, and deterministic encoder integration tests; the MSVC 19.44 + Ninja desktop build passed `30/30`. It remained unattached to writer/session/runtime/UART, and the next implementation stage was the SITL-only harness recorded below.
- 2026-07-18 SITL update: the isolated harness now uses the actual C++ estimator+encoder against exact `Copter-4.3.6` hash `0c5e999c` over localhost TCP with GPS disabled. Repeated runs confirmed the explicit global origin, exact parameters and frames, EKF ExternalNav acquisition (`flags=831`), disarmed GUIDED acceptance, reset counters, invalid-frame suppression, provider timeout (`flags=39`), and recovery. The early pre-provider request had no Home, but after acquisition ArduCopter automatically reported Home at origin; integer-coordinate `MAV_CMD_DO_SET_HOME` was ACK accepted, and the same Home persisted across timeout and recovery. MSVC/Ninja and WSL/Ninja pass `31/31`. The next stage is a separately reviewed props-off real-FC acceptance that first confirms current readiness and the same origin/Home sequence; do not attach to the existing runtime writer/UART or authorize armed/command output as part of that step. A later RC preflight trigger may reuse the JT_Zero operator interaction only after current RC mappings are read live, and must be disarmed-only; the old VO-only in-flight reset is not reusable as FC Home logic.
