...
- Status: started with targeted hardening. `BoundedNavigator` config validation now rejects non-finite values and negative yaw gain, and unit tests cover per-link health blocking, future match timestamps, invalid-match command reset, and invalid config rejection. `VHRS` route artifact parsing now rejects excessive entry counts before allocation, oversized payloads, and payload-size mismatches against dimensions plus pixel format.
