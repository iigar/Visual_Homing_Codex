...
- Status: next. Live MAVLink output remains blocked by default, blocked at CMake configure time, and blocked by the fail-closed `LiveMavlinkBridge` boundary until this milestone is completed and reviewed.
...
- Status: deferred until Milestone 6.7 is complete. Live route matching can now feed `BoundedNavigator` and `DryRunCommandSink` metrics under `VISUAL_HOMING_LIVE_ROUTE_DRY_RUN_COMMANDS=1`, optionally gate the run on dry-run command quality, optionally use read-only live MAVLink telemetry freshness as the dry-run health input, and still keep live MAVLink command output blocked. The validated `jtzero` compact dry-run captured 150/150 valid matches, passed endpoint/progress gates, passed telemetry health with zero dropped bytes, passed dry-run command quality with 150/150 valid commands, and kept future live output blocked with `vehicle_not_armed:150`. This is sufficient dry-run evidence to plan pre-live readiness, not to start the flight ladder.
