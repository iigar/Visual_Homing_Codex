...
- Status: dry-run bridge and telemetry adapter are implemented; replay route matching now uses scripted heartbeat telemetry to drive MAVLink health and maps yaw/altitude into navigation estimates. Next required step is enforcing armed/mode command permission gates in dry-run before any live ArduPilot output.
