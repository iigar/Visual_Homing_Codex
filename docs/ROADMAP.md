...
- Status: planned. The trigger evidence is the `2026-06-14` `jtzero` setup attempt where `ExternalNav` was configured but no provider existed, so armed `Guided` still failed with `requires position`.
