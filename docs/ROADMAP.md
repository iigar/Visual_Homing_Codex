...
- Status: started with targeted hardening. `BoundedNavigator` config validation now rejects non-finite values and negative yaw gain, and unit tests cover per-link health blocking, future match timestamps, invalid-match command reset, and invalid config rejection. `VHRS` route artifact parsing now rejects excessive entry counts before allocation, oversized payloads, and payload-size mismatches against dimensions plus pixel format. The read-only MAVLink telemetry stream now retains a bounded byte buffer and reports captured/retained/dropped byte metrics for longer-run memory auditing. Dry-run command sinks now retain bounded command history while preserving total sent/dropped counters. Live MAVLink command output is represented by an explicit fail-closed `LiveMavlinkBridge` stub that is unavailable by default and tested to reject sends; the `VISUAL_HOMING_ENABLE_LIVE_MAVLINK_OUTPUT` CMake option currently fails configuration until a reviewed live writer and safety gate contract replace the blocked boundary. Live-route active-profile CLI parsing now requires complete numeric values, and the Pi test script rejects non-`0`/`1` boolean env flags plus invalid expected-progress values. A standalone `LiveMavlinkOutputSafetyGate` now defines the pre-writer permission contract for runtime enable, operator confirmation, single writer, audit log, dry-run quality, fresh armed Guided telemetry, route-match freshness/confidence, and bounded finite commands.
...
- Status: started with opt-in live-route dry-run command logging and command-quality metrics. Live route matching can now feed `BoundedNavigator` and `DryRunCommandSink` metrics under `VISUAL_HOMING_LIVE_ROUTE_DRY_RUN_COMMANDS=1`, optionally gate the run on dry-run command quality, optionally use read-only live MAVLink telemetry freshness as the dry-run health input, and still keep live MAVLink command output blocked. The first `jtzero` forward dry-run with live read-only telemetry health passed with 150/150 valid matches, 150/150 valid dry-run commands, zero telemetry-degraded frames, and `passed=true`. A later bounded-buffer validation run confirmed telemetry and command quality again with `telemetry_bytes_dropped=0`; that run correctly returned `passed=false` because the hand-carried route pass did not reach endpoint progress and had local progress regressions. Live-route dry-run frame logs now also include future live-output safety gate diagnostics and final gate counters/reason while leaving real output blocked.
