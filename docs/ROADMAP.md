...
- 2026-07-18 update: the short outdoor route, forward/reverse matching, bounded provider-send evidence, read-only FC baseline, straight-route confirmation, exact-4.3.6 library-only route-local ODOMETRY encoder, and separate route-local estimator now exist. The estimator has explicit route-start vertical state, forward yaw and opt-in reverse-yaw semantics, reset-counter behavior, direction-change reset, bounded update-rate/freshness/progress/health gates, and deterministic encoder integration tests; the MSVC 19.44 + Ninja desktop build passed `30/30`. It remains unattached to writer/session/runtime/UART. The next implementation stage is a SITL-only harness that validates estimator plus exact ODOMETRY behavior without a real FC connection, including origin/home, mode readiness, reset, timestamp, frame, invalid-input, and recovery behavior. Do not repeat provider-send or attach the encoder until SITL/FC acceptance and safety gates are separately reviewed.
