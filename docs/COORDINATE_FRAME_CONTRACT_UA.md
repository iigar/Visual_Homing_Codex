...
Інакше причина є `frame_alignment_not_known` або `altitude_origin_not_aligned`. `LiveExternalNavOutputSession`, `LiveMavlinkExternalNavWriter` і прямий `VISION_POSITION_ESTIMATE` encoder повторно перевіряють цей контракт та fail closed. `ROUTE_FRD` або `LOCAL_ENU` не можуть бути неявно закодовані як NED.
