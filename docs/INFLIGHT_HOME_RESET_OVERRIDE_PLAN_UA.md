...
За цільовою архітектурою JT_Zero може залишатися локальним visual-odometry/hold anchor навіть тоді, коли глобальний route match відсутній. Він може обмежувати короткочасний знос і дати час на пошук, але не знає напрямку до маршруту. Visual Homing має окремо шукати compact descriptors по потоково записаному маршруту, перевіряти top-N через розріджені altitude/scale-tagged `1280x800` keyframes і вимагати multi-frame consistency. Одночасні незалежні external-nav publishers заборонені: handoff потребує single-owner policy, frame realignment і нового `reset_counter`.
