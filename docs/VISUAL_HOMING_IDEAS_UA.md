...
Accepted architecture direction on `2026-07-19`. Streaming writer/recorder, VHRM tracking chunks/recovery, VHIX descriptors та policy-only sparse verification/gate selector завершені на desktop. Selector має side-effect-free `evaluate()` і transactional `commit()`, обирає кадри за displacement/altitude/scale/yaw/scene/time та перевіряє local uncertainty, approach margin, separation і gate-to-gate novelty. WSL/MSVC проходять `42/42`. Реальний Pi/camera benchmark, native `1280x800` capture/storage, multiscale verification і JT_Zero handoff ще не виконані.
...
Перший library-only `VHRM v1` manifest/parser/writer уже моделює ці поля, SHA-256 package integrity та safe relative paths. Tracking builder ротує/recover-ить VHRS chunks, offline `VHIX v1` builder створює centered Gray8 block-mean descriptors та окремий indexed manifest, а transactional selector визначає verification requests і gate candidates без зміни стану до успішного capture/write commit; WSL/MSVC проходять `42/42`. VHIX/selector ще не підключені до native camera writer, matcher або runtime. Детальні контракти: `docs/ROUTE_PACKAGE_MANIFEST_V1_UA.md`, `docs/ROUTE_DESCRIPTOR_INDEX_V1_UA.md`, `docs/VERIFICATION_GATE_SELECTOR_UA.md`.
...
3. Спроєктувати route manifest/chunks/layers та sparse `1280x800` metadata/index/selection contract — VHRM, tracking builder/recovery, coarse VHIX descriptor/index і transactional selector виконані на desktop; native capture/storage ще попереду.
