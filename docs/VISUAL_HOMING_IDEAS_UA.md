...
Accepted architecture direction on `2026-07-19`. Streaming writer/recorder, VHRM tracking chunks/recovery, VHIX descriptors, sparse selector та native verification package writer завершені на desktop. Selector обирає кадри за displacement/altitude/scale/yaw/scene/time; writer публікує supplied native entry як immutable one-frame chunk і cumulative VHRM revision/gate до selector commit. WSL/MSVC проходять `43/43`. Реальний Pi/camera benchmark, live `1280x800` capture/synchronization, revision recovery, multiscale content verification і JT_Zero handoff ще не виконані.
...
3. Спроєктувати route manifest/chunks/layers та sparse `1280x800` metadata/index/selection/publication contract — VHRM, tracking builder/recovery, VHIX, selector і supplied-native-entry package writer виконані на desktop; live capture/synchronization та recovery ще попереду.
