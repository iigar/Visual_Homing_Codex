...
Accepted architecture direction on `2026-07-19`. Streaming writer/recorder, VHRM tracking chunks/recovery, VHIX descriptors, sparse selector, native verification package writer, `LiveVerificationCaptureSession` та `BoundedVerificationPublisher` завершені. Session прив'язує native entry і VHIX descriptor до одного camera-owned `frame_id/timestamp`; publisher виконує її в single worker із bounded active+queued capacity та explicit backpressure/failure/drain state. Desktop WSL і Pi all-output-off baseline тепер `45/45`, MSVC target tests проходять. Повторний 600-second Pi Zero 2W/OV9281 async benchmark прийняв native `1280x800` при sparse cadence `10 s`: `60/60` verified publications, `8439/8439` accepted/completed, `1514` explicit backpressure, max outstanding `2`, zero failed/abandoned/discarded work, camera-loop `16.5967 fps`, RSS max `25580 KiB`, temperature max `61.224 °C`, `get_throttled=0x0`. Revision/restart recovery, multiscale content verification, global search і JT_Zero handoff ще не виконані.
...
Перший library-only `VHRM v1` manifest/parser/writer уже моделює ці поля, SHA-256 package integrity та safe relative paths. Tracking builder ротує/recover-ить VHRS chunks, offline `VHIX v1` builder створює descriptors/indexed manifest, selector визначає verification requests/gates, а `VerificationPackageWriter` публікує supplied native entries і immutable VHRM revisions до commit. Сам writer не відкриває camera, але `LiveVerificationCaptureSession` виконує camera-frame/descriptor/native-entry зв'язування, а `BoundedVerificationPublisher` ізолює цей synchronous transaction у bounded worker. Desktop WSL і Pi all-output-off suites проходять `45/45`, MSVC target tests проходять; async Pi acceptance завершено з `60/60` publications і explicit bounded backpressure без failure. VHIX усе ще не має search consumer, а production route attachment залишається окремим наступним composition step. Детальні контракти: `docs/ROUTE_PACKAGE_MANIFEST_V1_UA.md`, `docs/ROUTE_DESCRIPTOR_INDEX_V1_UA.md`, `docs/VERIFICATION_GATE_SELECTOR_UA.md`, `docs/VERIFICATION_PACKAGE_WRITER_UA.md`, `docs/LIVE_VERIFICATION_CAPTURE_UA.md`.

## Поточний Найближчий План

1. Library-only streaming `VHRS v1` writer без camera/runtime caller — виконано (`37/37`).
2. Bounded streaming recorder integration для live route, зі збереженим in-memory replay recorder — library/desktop integration і Pi async publisher acceptance виконані; desktop/Pi baseline `45/45`, production route composition ще не виконано.
