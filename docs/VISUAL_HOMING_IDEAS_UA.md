...
Accepted architecture direction on `2026-07-19`. Streaming writer/recorder, VHRM tracking chunks/recovery, VHIX descriptors, sparse selector, native verification package writer, `LiveVerificationCaptureSession`, `BoundedVerificationPublisher` і fail-closed `LiveRouteVerificationProducer` завершені на library level. Session прив'язує native entry і VHIX descriptor до одного camera-owned `frame_id/timestamp`; publisher виконує її в single worker із bounded active+queued capacity та explicit backpressure/failure/drain state; producer перед worker перевіряє same-frame match, tracked progress, health, fresh scalars і optional named local pose. Desktop WSL і clean Pi all-output-off baselines тепер `46/46`, MSVC affected tests проходять. Повторний 600-second Pi Zero 2W/OV9281 async benchmark прийняв native `1280x800` при sparse cadence `10 s`: `60/60` verified publications, `8439/8439` accepted/completed, `1514` explicit backpressure, max outstanding `2`, zero failed/abandoned/discarded work, camera-loop `16.5967 fps`, RSS max `25580 KiB`, temperature max `61.224 °C`, `get_throttled=0x0`. Operational matcher/CLI caller, trusted local pose, revision/restart recovery, multiscale content verification, global search і JT_Zero handoff ще не виконані.
