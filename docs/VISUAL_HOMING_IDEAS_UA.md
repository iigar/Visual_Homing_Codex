...
Перший library-only `VHRM v1` manifest/parser/writer вже моделює ці поля, SHA-256 package integrity та safe relative paths. Tracking-only builder тепер автоматично ротує bounded VHRS chunks, відновлює лише checkpointed prefix і фіналізує manifest після digest; WSL/MSVC проходять `40/40`. Він ще не створює descriptors/verification chunks і не підключений до camera recorder, matcher або runtime. Детальний контракт: `docs/ROUTE_PACKAGE_MANIFEST_V1_UA.md`.
...
3. Спроєктувати route manifest/chunks/layers та sparse `1280x800` metadata/index contract — library-only `VHRM v1` і tracking chunk builder/recovery виконані на desktop; descriptor/index algorithm ще попереду.
