...
Accepted architecture direction on `2026-07-19`. Streaming writer/recorder, VHRM tracking chunks/recovery, VHIX descriptors, sparse selector, native verification package writer та `LiveVerificationCaptureSession` завершені. Session прив'язує native entry і VHIX descriptor до одного camera-owned `frame_id/timestamp`; desktop WSL/MSVC та Pi проходять `44/44`. Перший 600-second Pi Zero 2W/OV9281 benchmark прийняв native `1280x800` при sparse cadence `10 s`: `6679` observed frames, `60/60` verified publications, bounded RSS/temperature, `get_throttled=0x0` і нуль failures. Синхронна cumulative publication водночас зросла до `3072.75 ms`, тому production tracking attachment відхилено до появи bounded background publisher. Revision/restart recovery, multiscale content verification, global search і JT_Zero handoff ще не виконані.
...
Перший library-only `VHRM v1` manifest/parser/writer уже моделює ці поля, SHA-256 package integrity та safe relative paths. Tracking builder ротує/recover-ить VHRS chunks, offline `VHIX v1` builder створює descriptors/indexed manifest, selector визначає verification requests/gates, а `VerificationPackageWriter` публікує supplied native entries і immutable VHRM revisions до commit. Сам writer не відкриває camera, але `LiveVerificationCaptureSession` уже виконує синхронне camera-frame/descriptor/native-entry зв'язування; desktop WSL/MSVC та Pi проходять `44/44`, а перший live Pi capture benchmark прийнято. VHIX усе ще не має search consumer, а синхронний publication path не дозволений у production runtime до bounded background worker. Детальні контракти: `docs/ROUTE_PACKAGE_MANIFEST_V1_UA.md`, `docs/ROUTE_DESCRIPTOR_INDEX_V1_UA.md`, `docs/VERIFICATION_GATE_SELECTOR_UA.md`, `docs/VERIFICATION_PACKAGE_WRITER_UA.md`, `docs/LIVE_VERIFICATION_CAPTURE_UA.md`.

## Поточний Найближчий План

1. Library-only streaming `VHRS v1` writer без camera/runtime caller — виконано (`37/37`).
2. Bounded streaming recorder integration для live route, зі збереженим in-memory replay recorder — library/desktop integration виконано; поточний загальний desktop/Pi baseline `44/44`, production route composition ще не виконано.
3. Спроєктувати route manifest/chunks/layers та sparse `1280x800` metadata/index/selection/publication contract — VHRM, tracking builder/recovery, VHIX, selector, supplied-native-entry package writer і live frame synchronization виконані; bounded async publication та restart/revision recovery ще попереду.
4. Провести Pi thermal/load/storage benchmark перед вибором максимальної sparse-keyframe частоти — перший `1280x800`/`10 s`/600 s benchmark виконано й прийнято як capacity evidence; повторити після async publisher перед production cadence decision.
