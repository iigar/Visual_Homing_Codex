...
- Pi 10-15 minute recording matrix для target sizes і sparse `1280x800` rate — перша 600-second точка `1280x800`/`10 s` виконана; повторити після bounded async publisher;
...
Accepted architecture direction on `2026-07-19`. Streaming writer/recorder, VHRM tracking chunks/recovery, VHIX descriptors, sparse selector, native verification package writer, `LiveVerificationCaptureSession` та desktop `BoundedVerificationPublisher` завершені. Session прив'язує native entry і VHIX descriptor до одного camera-owned `frame_id/timestamp`; publisher виконує її в single worker із bounded active+queued capacity та explicit backpressure/failure/drain state. Desktop full WSL baseline тепер `45/45`, MSVC target tests проходять; історичний Pi baseline залишається `44/44`. Перший 600-second Pi Zero 2W/OV9281 benchmark прийняв native `1280x800` при sparse cadence `10 s`: `6679` observed frames, `60/60` verified publications, bounded RSS/temperature, `get_throttled=0x0` і нуль failures. Виміряний synchronous blocker до `3072.75 ms` усунуто архітектурно на desktop, але потрібен повторний Pi run оновленого async benchmark. Revision/restart recovery, multiscale content verification, global search і JT_Zero handoff ще не виконані.
...
Перший library-only `VHRM v1` manifest/parser/writer уже моделює ці поля, SHA-256 package integrity та safe relative paths. Tracking builder ротує/recover-ить VHRS chunks, offline `VHIX v1` builder створює descriptors/indexed manifest, selector визначає verification requests/gates, а `VerificationPackageWriter` публікує supplied native entries і immutable VHRM revisions до commit. Сам writer не відкриває camera, але `LiveVerificationCaptureSession` виконує camera-frame/descriptor/native-entry зв'язування, а `BoundedVerificationPublisher` ізолює цей synchronous transaction у bounded worker. Desktop WSL full suite проходить `45/45`, MSVC target tests проходять, історичний Pi baseline має `44/44`; async Pi re-benchmark ще не виконано. VHIX усе ще не має search consumer, а production route attachment не дозволений до Pi acceptance. Детальні контракти: `docs/ROUTE_PACKAGE_MANIFEST_V1_UA.md`, `docs/ROUTE_DESCRIPTOR_INDEX_V1_UA.md`, `docs/VERIFICATION_GATE_SELECTOR_UA.md`, `docs/VERIFICATION_PACKAGE_WRITER_UA.md`, `docs/LIVE_VERIFICATION_CAPTURE_UA.md`.

## Поточний Найближчий План

1. Library-only streaming `VHRS v1` writer без camera/runtime caller — виконано (`37/37`).
2. Bounded streaming recorder integration для live route, зі збереженим in-memory replay recorder — library/desktop integration виконано; desktop WSL full baseline `45/45`, історичний Pi baseline `44/44`, production route composition ще не виконано.
...
4. Провести Pi thermal/load/storage benchmark перед вибором максимальної sparse-keyframe частоти — перший synchronous `1280x800`/`10 s`/600 s benchmark виконано й прийнято як capacity evidence; наступний безпосередній крок — повторити його з async publisher перед production cadence decision.
