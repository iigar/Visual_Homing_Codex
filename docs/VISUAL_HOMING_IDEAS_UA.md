...
Перший library-only `VHRM v1` manifest/parser/writer уже моделює ці поля, SHA-256 package integrity та safe relative paths. Tracking builder ротує/recover-ить VHRS chunks, а offline `VHIX v1` builder створює centered Gray8 block-mean descriptors та окремий indexed manifest; WSL/MSVC проходять `41/41`. VHIX поки лише дає coarse candidates і не підключений до camera recorder, matcher або runtime. Детальні контракти: `docs/ROUTE_PACKAGE_MANIFEST_V1_UA.md`, `docs/ROUTE_DESCRIPTOR_INDEX_V1_UA.md`.
...
3. Спроєктувати route manifest/chunks/layers та sparse `1280x800` metadata/index contract — VHRM, tracking builder/recovery і coarse VHIX descriptor/index виконані на desktop; sparse verification/gate selection ще попереду.
