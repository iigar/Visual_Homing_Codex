...
Pi Zero 2W synchronous benchmark із `60` native `1280x800` revisions підтвердив correctness/cadence без throttling, але також виміряв зростання publication від `110.024 ms` до `3072.75 ms`. Це remediation уже закрито `BoundedVerificationPublisher`: повторний 600-second run прийняв `60/60` revisions, explicit backpressure і max outstanding `2` без worker failure або throttling. Operational producer-to-matcher/CLI wiring, revision resume/durability та search/verification залишаються відкритими. Evidence: `docs/PI_VERIFICATION_CAPTURE_BENCHMARK_2026-07-19_UA.md`, `docs/PI_VERIFICATION_CAPTURE_ASYNC_BENCHMARK_2026-07-27_UA.md`; current order: `docs/CURRENT_PROJECT_STATUS_UA.md`.
