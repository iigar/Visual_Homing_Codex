...
- Остання C++ desktop валідація: після синхронізованого lazy edge cache Debug і Release по `54/54` (інкрементально на попередніх чистих builds); усі сім `VISUAL_HOMING_*` flags OFF; shell `15/15`, benchmark helpers `5/5`; окремий ThreadSanitizer test пройшов. `60` CLI cases входять в один CTest-сценарій. `scripts/test-software.sh` запускає обидві Python suites. Pi/MSVC/hardware acceptance цим не підтверджено.
...
- Lazy edge-cache переглянуто: несинхронізовану незакомічену чернетку замінено одноразовою синхронізованою побудовою. RSS і перший edge-виклик виміряно проти ізольованого `a33217a`; артефакти `artifacts/edge-cache-review-20260922/`, методика в `EDGE_CACHE_OPTIMIZATION_2026-09-22_UA.md`. Наступний обмежений software крок — перевірити еквівалентний early exit scale refinement. Точність і пороги за self-кадрами не оцінювати; незалежна оцінка потребує повного другого проходу з розміткою. До запуску камери або апаратної перевірки як частини software slice не переходити.
