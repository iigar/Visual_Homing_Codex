...
- Offline integration (`2026-09-26`): 24 сценарії зі справжніми matcher → progress → endpoint → quality/readiness API, синтетичними кадрами та контрольованими timestamps. Forward/reverse, global/window/scale, невалідний збіг, raw/edge ambiguity, неповний прохід і старі кадри; Debug/Release `58/58`, Python `15/15` і `5/5`. Production policy не змінена. Підтверджено: invalid gap зберігає dwell, але блокує загальний route pass; route-only pass не має age-gate. Це композиція API, не виконання camera runtime або downstream freshness gates. [Звіт, покриття й межі](OFFLINE_INTEGRATION_VALIDATION_2026-09-26_UA.md).
...
- Остання C++ desktop валідація: після offline integration test Debug і Release по `58/58` (інкрементально на попередніх чистих builds); усі сім `VISUAL_HOMING_*` flags OFF; shell `15/15`, benchmark helpers `5/5`; `BUILD_TESTING=OFF` успішний із 0 тестів. Raw-distance ASan/UBSan evidence — із попереднього optimization slice, scale-kernel ASan/UBSan — від `2026-09-25`, edge-cache ThreadSanitizer — від `2026-09-22`; новий інтеграційний test sanitizer не запускав. `60` CLI cases входять в один CTest-сценарій. `scripts/test-software.sh` запускає обидві Python suites. Pi/MSVC/hardware acceptance цим не підтверджено.
...
- Raw distance optimization завершено `2026-09-26` й опубліковано як `a5513da`: часткові суми блоками до 65536 пікселів у `uint32_t`, загальна сума `uint64_t`, без heap/cache чи зміни формули. На PGM зі refinement медіана `3.504 → 1.381 ms`; усі 44 сценарії та edge-діагностика шести маршрутів збігаються. 65622 API cases включають суму понад `UINT32_MAX`; артефакти `artifacts/raw-distance-20260925/`, методика в `RAW_DISTANCE_OPTIMIZATION_2026-09-26_UA.md`.
- Offline API integration завершено `2026-09-26`: новий test-only fixture викликає справжні matcher/progress/endpoint/readiness та HealthMonitor на 24 сценаріях. Закрито прогалину між окремими unit tests; production-код не змінений. Invalid gap не скидає dwell, але invalid frame робить route session непрохідною; route-only readiness не має перевірки віку кадру. Це не перевірка повного camera loop. Артефакти `artifacts/offline-integration-20260926/`, межі в `OFFLINE_INTEGRATION_VALIDATION_2026-09-26_UA.md`.
- Наступний обмежений software крок — перевірити stale/invalid observations у фактичних downstream consumers `BoundedNavigator` і progress-only verification: інвентаризація тестів, контрольовані time/health сценарії та звірення меж із route-only reporting. Не змінювати policy й не підключати output у цьому кроці. Після оптимізації профіль `match`: raw distance близько 43%, scale kernel 53%, candidates 1.24%; окрема оптимізація вставки кандидатів зараз не обґрунтована. Не переносити весь camera runtime. Точність і пороги за self-кадрами не оцінювати; незалежна оцінка потребує повного другого проходу з розміткою. До запуску камери або апаратної перевірки як частини software slice не переходити.
