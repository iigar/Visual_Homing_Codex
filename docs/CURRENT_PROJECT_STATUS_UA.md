...
### Точка Відновлення 2026-09-21

- Попередні CMake/shell/endpoint/readiness зміни зафіксовано в `5793361` після `fff1361`. Наступний slice додає numeric config fix, два CTest-сценарії, спільну команду `scripts/test-software.sh` і звіт. Гілка залишається `refactor/optimization-tech-debt`; актуальний commit перевіряти через `git log`/`git status`.
- Остання C++ desktop валідація: чисті Debug і Release по `52/52`, усі сім `VISUAL_HOMING_*` flags OFF; shell `15/15`. `60` CLI cases входять в один CTest-сценарій, а не додаються до 52 окремо. Pi/MSVC/hardware acceptance цим не підтверджено.
- При відновленні прочитати цей snapshot, `SOFTWARE_VALIDATION_2026-09-21_UA.md` і найновіший запис `SESSION_LOG.md`. Перевірити наявність відповіді користувача про місце зберігання повного другого проходу; локально в `artifacts/`, `keyframes/`, `config/` знайдено лише 37 окремих PGM, без підтвердженого повного розміченого запису. До апаратних перевірок як частини software slice не переходити.
