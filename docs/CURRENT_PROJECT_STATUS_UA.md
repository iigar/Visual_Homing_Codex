...
- Локальний matcher benchmark (`2026-09-22`): усі `2583/2583` власні кадри шести маршрутів точно зіставлено у трьох режимах; результати трьох повторів стабільні. WSL Release window ±30 має p50 `0.238–0.244 ms`, із scale refinement — `11.288–11.915 ms`. Для `11/13` stop-frame знімків глобальний індекс відрізняється від історичного stateful результату; це не error rate без незалежної розмітки. Debug/Release після додавання інструмента по `53/53`, shell `15/15`, benchmark helpers `5/5`; усі output/camera flags OFF. [Методика, числа та обмеження](MATCHER_BENCHMARK_2026-09-22_UA.md).
...
- Остання C++ desktop валідація: після додавання окремого matcher benchmark Debug і Release по `53/53` (інкрементально на попередніх чистих builds); усі сім `VISUAL_HOMING_*` flags OFF; shell `15/15`, benchmark helpers `5/5`. `60` CLI cases входять в один CTest-сценарій. `scripts/test-software.sh` запускає обидві Python suites. Pi/MSVC/hardware acceptance цим не підтверджено.
...
- Локальний benchmark завершено: `scripts/benchmark-recorded-routes.py`, окремий `gray8_matcher_benchmark`, сирі результати `artifacts/matcher-benchmark-20260922/run1/`. Production matcher незмінений. Один маршрут (`20260710T154326Z`) не проходить наявну distinctiveness policy; self-match усіх маршрутів проходить. Борт для подальшої локальної роботи можна вимкнути.
- При відновленні прочитати цей snapshot, `MATCHER_BENCHMARK_2026-09-22_UA.md`, `BOARD_RECORDING_INVENTORY_2026-09-22_UA.md` і найновіший запис `SESSION_LOG.md`. Наступний обмежений software крок — виміряти edge-cache memory/init або перевірити еквівалентний early exit scale refinement; точність і пороги за self-кадрами не оцінювати. Незалежна оцінка потребує повного другого проходу з розміткою. До запуску камери або апаратної перевірки як частини software slice не переходити.
