...
- Near-term priority: validate route distinctiveness diagnostics on the persistent `jtzero` live `VHRS` artifact, then decide whether Milestone 6 is complete enough to move to bench replay/flight-test ladder preparation; live MAVLink output remains explicitly blocked and desktop builds must keep replay-first tests working and fail closed without camera backend.
