...
- Near-term priority: decide whether Milestone 6 is complete enough to move to bench replay/flight-test ladder preparation. The latest static table captures correctly report poor route distinctiveness, while a hand-carried 180-frame bench route improved to 8/180 ambiguous nearest entries with average nearest mean absolute byte difference ~7.44 and passed structure/self-match/perturbation checks. Future field tests should still use textured motion/scene variation or add stronger route-quality gates. Live MAVLink output remains explicitly blocked and desktop builds must keep replay-first tests working and fail closed without camera backend.
