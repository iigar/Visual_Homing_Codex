...
- Near-term priority: validate `--inspect-route` on the `jtzero` live `VHRS` artifact, then add a deterministic live-recording artifact check; desktop builds must keep replay-first tests working and fail closed without camera backend.
