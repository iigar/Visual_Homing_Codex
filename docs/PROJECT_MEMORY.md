...
- Near-term priority: decide whether Milestone 6 is complete enough to move to bench replay/flight-test ladder preparation. The latest static table captures correctly report poor route distinctiveness, while a hand-carried 180-frame bench route improved to 8/180 ambiguous nearest entries with average nearest mean absolute byte difference ~7.44, passed structure/self-match/perturbation checks, and satisfies the initial route-quality policy with `quality_pass=true`. Live MAVLink output remains explicitly blocked and desktop builds must keep replay-first tests working and fail closed without camera backend.
