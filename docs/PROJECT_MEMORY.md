...
- Near-term priority: decide whether Milestone 6 is complete enough to move to bench replay/flight-test ladder preparation; the latest `jtzero` live artifacts pass structure/self-match/perturbation checks but route distinctiveness reports `warning=true` because most entries are visually ambiguous, so future field tests should use more textured motion/scene variation or add stronger route-quality gates. Live MAVLink output remains explicitly blocked and desktop builds must keep replay-first tests working and fail closed without camera backend.
