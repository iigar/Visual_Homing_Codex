...
- Near-term priority: keep live MAVLink output blocked while preparing the follow-up reviewed implementation plan for the first bench props-off live-output boundary. `docs/LIVE_OUTPUT_SAFETY_PLAN.md` is the controlling safety artifact: first future live-output boundary is bench-only with propellers removed, first command scope is yaw-rate-only with zero forward velocity, audit logging must be enabled and ready before any command can pass the safety gate, and 3/3 clean Pi dry-runs are now recorded in `docs/LIVE_OUTPUT_READINESS_RECORD.md`. The third accepted run used `scripts/check-route-quality-log.sh` on a new route before the match/audit run, then passed `scripts/check-live-readiness-log.sh` and `scripts/check-live-session-audit-log.sh`. Remaining non-live work includes timestamp-aligned continuous MAVLink telemetry integration, an HTTP wrapper over the profile registry, altitude-aware route metadata, thermal normalization/profile support, and designing the reviewed bench props-off writer boundary without enabling flight. Milestone 7 flight ladder is deferred until the bench props-off live-output boundary is separately implemented and reviewed.
