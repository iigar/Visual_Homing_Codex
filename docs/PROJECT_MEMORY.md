...
- Current validated core baseline: replay pipeline plus route signature v1, coarse matching, initial navigation command model, dry-run MAVLink boundary, and initial hardware capture boundary has `ReplayFrameSource`, `Gray8ResizePreprocessor`, `HealthMonitor`, `PipelineHarness`, binary `VHRS` route signature writer/reader, route inspection summary, `RouteSignatureRecorder`, `Gray8RouteMatcher`, `BoundedNavigator` with yaw-rate slew limiting, `DryRunCommandSink`, `DryRunMavlinkBridge`, `MavlinkTelemetryAdapter` with heartbeat freshness plus armed/Guided permission gates, `PiCameraSource` with initial libcamera backend behind compile-time `VISUAL_HOMING_ENABLE_LIBCAMERA` and runtime `enable_live_capture`, live camera pipeline smoke, live camera route recording, and route artifact inspection validated on `jtzero`, replay-to-route matching CLI with progress/confidence/direction error/health gate/command output metrics, MSVC/CMake build support, Raspberry Pi bootstrap/test scripts validated on Pi Zero 2W class hardware, and 13 passing CTest tests.
- Near-term priority: add a deterministic live-recording artifact check and then start offline self-match/perturbation checks against recorded `VHRS` artifacts; desktop builds must keep replay-first tests working and fail closed without camera backend.
