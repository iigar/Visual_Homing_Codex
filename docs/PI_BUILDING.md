...
For operator readiness, use `external_nav_operator_readiness=ready|marginal|blocked` and `external_nav_operator_reason`. `blocked` means the tolerant log-quality contract failed. `marginal` means the safety/readiness gates passed but a diagnostic was not clean, such as tracked route directional progress exceeding the operator soft threshold or the strict all-frames external-nav diagnostic. The log keeps raw route progress diagnostics (`progress_regressions`, `reverse_progress_regressions`, and rollback totals) and also reports `tracked_progress`, `tracked_minmax_progress`, `tracked_regressions`, `tracked_rollback`, and `tracked_directional_progress`. The tracked progress is a dry-run-only directional envelope: for `reverse` it may move toward lower progress but ignores short raw jumps toward higher progress, and for `forward` it may move toward higher progress but ignores short raw jumps toward lower progress. Raw fields still expose vibration/pitch/viewpoint jitter. Tracked rollback accumulates opposite-direction motion that remains after the envelope, while tracked regression counts ignore micro-corrections at or below `0.01` route-progress units. The operator route-progress soft threshold is intentionally looser than the core directional gate: up to `15` tracked progress regressions and `1.0` tracked total rollback can still be `ready` when endpoint and quality gates pass. `ready` means both the tolerant quality gate and the currently tracked soft diagnostics passed. The lower-level `external_nav_session_ready` and `external_nav_quality_ready` fields still expose the tolerant log-quality contract directly: route session passed, required telemetry health passed, enough FC-ready external-nav estimates were produced, invalid streaks stayed short, and optional altitude/visual-scale gates passed. The strict all-frames diagnostic is reported separately as `external_nav_strict_session_ready`; it can be false for small route-match glitches such as `148/150` valid estimates while the tolerant session remains valid.
...
If preflight altitude sanity fails, the wrapper stops before camera route matching and still writes the configured readiness JSON path with `stage=preflight`, `operator.readiness=blocked`, and a `preflight` object describing the blocker. If live route matching starts but fails after emitting `live_route_match_compact`, the wrapper still exports the current run JSON before exiting. This prevents Android or the operator from accidentally reading an older successful dry-run JSON after a current preflight or route failure.
